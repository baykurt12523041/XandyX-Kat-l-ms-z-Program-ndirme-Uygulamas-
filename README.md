<div align="center">

<!-- Header Section -->

<br />
<img src="https://i.ibb.co/pjHVH1Jj/X.png" width="120" alt="XandyX Logo" />

<font face="Plus Jakarta Sans">🚀 XandyX</font>

<b>Yeni Nesil Katılımsız Yazılım Platformu</b>



XandyX, Windows yazılımlarını saniyeler içinde, sıfır müdahale ve maksimum optimizasyon ile kuran hibrit bir çözüm platformudur.

<p align="center">
<a href="https://byxandyx.rf.gd"><b>🌐 Web Sitesi</b></a> •
<a href="https://github.com/baykurt12523041/XandyX-Kat-l-ms-z-Program-ndirme-Uygulamas-/releases"><b>📦 İndir</b></a> •
<a href="https://byxandyx.rf.gd"><b>🛠 Özellikler</b></a> •
<a href="https://www.google.com/search?q=https://github.com/kullaniciadi/xandyx/issues"><b>🤝 Katkıda Bulun</b></a>
</p>

</div>

<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Light%2520Bulb.png" width="25" /> ✨ Temel Özellikler

<table width="100%">
<tr>
<td width="50%" valign="top">
<h4>⚡ Ultra Hızlı</h4>
Kurulum parametreleri sayesinde "İleri" butonuna veda edin. Maksimum zaman tasarrufu.
</td>
<td width="50%" valign="top">
<h4>🛡️ Saf Deneyim</h4>
Reklam (Adware) veya gereksiz araç çubukları içermez. Sadece temiz yazılım.
</td>
</tr>
<tr>
<td width="50%" valign="top">
<h4>🧩 Modüler Tasarım</h4>
Sadece ihtiyacınız olan paketleri seçin ve kurun. Sisteminizde gereksiz dosya bırakmaz.
</td>
<td width="50%" valign="top">
<h4>🎨 Modern Arayüz</h4>
Plus Jakarta Sans fontu ve Bento Grid estetiği ile kullanıcı dostu bir deneyim.
</td>
</tr>
</table>

<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Camera%2520with%2520Flash.png" width="25" /> 📸 Ekran Görüntüleri

<div align="center">
<br />
<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/kullaniciadi/xandyx/main/preview.png" width="100%" alt="XandyX Preview" style="border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" />
<p><i>Modern Bento Grid yapısına sahip web arayüzümüzden bir kesit.</i></p>
</div>

<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Gear.png" width="25" /> 🛠 Teknik Detaylar

<div align="center">

Özellik

Açıklama

Geliştirme Yılı

2026

Desteklenen OS

Windows 10 / 11 (x64)

Dosya Boyutu

~450 MB

Dosya Adı

XandyX_Katilimsiz.exe

Mimari

.NET Framework 4.8+

</div>

<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Inbox%2520Tray.png" width="25" /> 📥 İndirme ve Kurulum


Projenin en son sürümünü kullanmak için aşağıdaki adımları takip edin.

İndir: XandyX_Katilimsiz.exe v2026.1.0 dosyasını resmi sitemizden indirin.

Çalıştır: İndirilen dosyaya sağ tıklayıp "Yönetici Olarak Çalıştır" seçeneğini kullanın.

Keyfini Çıkar: İstediğiniz programları işaretleyin, başlatın ve arkanıza yaslanın.

<img src="https://www.google.com/search?q=https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Shield.png" width="25" /> 🛡️ Güvenlik ve Şeffaflık

VirusTotal: Tüm paketler düzenli olarak taranmakta ve sonuçlar paylaşılmaktadır.

Bozulmamış Dosyalar: Orijinal üretici dosyaları üzerinde asla modifiye yapılmaz.

Zararsız Yapı: Kayıt defteri (Registry) üzerinde sadece gerekli optimizasyonları yapar.

<div align="center">

🤝 Katkıda Bulunma

Depoyu Fork ederek veya bir Issue açarak projenin gelişimine destek olabilirsiniz.

<br />

Estetik ve Fonksiyonun buluşma noktası.
<br />
© 2026 XandyX Project

</div>
